...
public class BusStyleRenderer : ISkiaStyleRenderer
{
    ...
    private readonly SKPaint _textPaint = new() { Color = SKColors.White };
        ...
        long iteration)
    {
        ...
        if (layer is not AnimatedPointLayer { DataSource: BusPointProvider })
            return false;
        ...
        canvas.DrawText(feature["number"]?.ToString(), 0, radius / 2, SKTextAlign.Center, new SKFont { Size = radius}, _textPaint);
        canvas.Restore();
